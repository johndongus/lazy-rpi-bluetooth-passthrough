...
def intercept():
    global state,bus,btkservice,iface
    bus = dbus.SystemBus()
    btkservice = bus.get_object('org.thanhle.btkbservice', '/org/thanhle/btkbservice')
    iface = dbus.Interface(btkservice, 'org.thanhle.btkbservice')
    ...
    while True:
        ...
        Y = float(Y)
        speed = 1
        X = min(127, max(-127, int(X * speed))) & 255
        Y = min(127, max(-127, int(Y * speed))) & 255
        iface.send_mouse(0, [state[0],X,Y,state[3]])
...
if __name__ == "__main__":
    InputDevice.init()
    threading.Thread(target=intercept).start()
    while True:
